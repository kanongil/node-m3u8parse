...
exports = module.exports = function (input, options = {}, cb) {
    ...
    const extParser = {
        ...
        '#EXT-X-DATERANGE': (arg) => {
            ...
            m3u8.meta.ranges.push(new AttrList(arg));
        },
        '#EXT-X-GAP': () => {
...
internals.stringifyAttrs = function (attrs) {
...
exports.M3U8Playlist = class {
    ...
    constructor(obj) {
        ...
        this.meta = {};
        if (obj.meta && obj.meta.ranges) {
            this.meta.ranges = internals.cloneAttrArray(obj.meta.ranges); // V7
        }
    ...
    toString() {
        ...
        for (const key of this.defines) {   // soft V8
            m3u8 += `#EXT-X-DEFINE:${internals.stringifyAttrs(key)}\n`;
        }
        ...
        if (this.master) {
            this.session_keys.forEach((key) => {
                ...
                m3u8 += `#EXT-X-SESSION-KEY:${internals.stringifyAttrs(key)}\n`;
            });
            ...
            if (this.keys && Array.isArray(this.keys)) {
                this.keys.forEach((key) => {
                    ...
                    m3u8 += `#EXT-X-KEY:${internals.stringifyAttrs(key)}\n`;
                });
            }
        }
        else {
            ...
            for (const key of this.meta.ranges || []) {   // soft V7
                m3u8 += `#EXT-X-DATERANGE:${internals.stringifyAttrs(key)}\n`;
            }
            ...
            if (this.type !== this.PlaylistType.VOD && this.type !== this.PlaylistType.EVENT) {
                const discontinuitySequence = parseInt(this.discontinuity_sequence, 10);
                if (!isNaN(discontinuitySequence)) {
        ...
        if (this.start && Object.keys(this.start).length) {
            m3u8 += `#EXT-X-START:${internals.stringifyAttrs(this.start)}\n`;
        } // soft V6
        ...
        if (this.master) {
            for (const list of this.data.values()) {  // soft V7
                for (const data of list) {
                    m3u8 += `#EXT-X-SESSION-DATA:${internals.stringifyAttrs(data)}\n`;
                }
            }

            for (const list of this.groups.values()) {
                for (const group of list) {
                    m3u8 += `#EXT-X-MEDIA:${internals.stringifyAttrs(group)}\n`;
                }
            }
    ...
    toString() {
        ...
        if (this.keys) {
            this.keys.forEach((key) => {
                ...
                res += `#EXT-X-KEY:${internals.stringifyAttrs(key)}\n`;
            });
        }

        if (this.map) {
            res += `#EXT-X-MAP:${internals.stringifyAttrs(this.map)}\n`;
        }
        ...
        if (this.duration) {
            res += `#EXTINF:${parseFloat(this.duration.toFixed(3))},${this.title}\n`;
