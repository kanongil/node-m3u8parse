...
exports = module.exports = function (input, options = {}, cb) {
    ...
    Assert.ok(internals.isStringish(input), 'Input must be a stream, string, or buffer');
...
exports.M3U8Playlist = class {
    ...
    rewriteUris(mapFn) {
        ...
        const rewriteAttr = (attrs, type) => {
            ...
            if (attrs && attrs.uri) {
                const newUri = mapFn(attrs.quotedString('uri'), type, attrs);
                if (internals.isStringish(newUri)) {
                    attrs.quotedString('uri', newUri);
                }
            }
        };
